module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		vagrant_commands: {
			...
			flush_cache: {
				commands: [
					['ssh', '--command=cd /var/sites/<%= site.id %> && bash scripts/flush_cache.sh']
				]
			},
			flush_mem: {
				commands: [
					['ssh', '--command=cd /var/sites/<%= site.id %> && bash scripts/flush_mem.sh']
				]
			}
		}
	});
	...
	grunt.registerTask('default', [ <%
		if (remoteDatabase) { %> 'http', <%
		} %> <%
		if (repos || dependencies) { %> 'gitPull', <%
		} %> <%
		if (svn_repos || svn_dependencies) { %> 'svn_checkout', <%
		} %> 'vagrant_commands:restart'
	]); <%
	...
	grunt.registerTask('relink', [ <%
		if (dependencies) { %> 'gitPull:dependencies', <%
		} %> <%
		if (svn_dependencies) { %> 'svn_checkout:dependencies', <%
		} %> 'vagrant_commands:symlinks'
	]);
